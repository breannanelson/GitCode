...
var state;
var keyword;
var mapData = database.ref("/location")
...
function initMap() {
  ...
  $('#submitBTN').on('click', function () {
    ...
    var contentString =       '<p>Working with :<b> ' + languages + '</b></p> ' +
      ...
      if (status == 'OK') {
        ...
        $.ajax({
          ...
          for (var i = 0; i < resultsLength; i++) {
            ...
            var objName = $('<button id="chatroomBTNS" value='+ objLocation +'>' +
            '<img src=" ' + icon + '" width="100px" height="100px"> ' + 
            '<div>'+ objDesc + '</div>' +
            '<div>'+ objLocation +'</div></button><br>')
            $('#diffChatrooms').append(objName);
            //create this 

            $(this).on('click', function(){
              console.log(this)
              //to create new chatroom for this object
              $("#mapSpace").css("display", "none");
              $("#messagePage").css("display", "block");
              $("#chat-messages-all").css("display", "none");
              $("#messagePage").append("<div id = 'chat-messages-" + placeLoc + "' style='display: block;''></div>");
            })

            localData.on("child_added", function (snapshot) {
              var msg = snapshot.val();
              $("#chat-messages-" + placeLoc + "").append(msg.displayName + " :  " + msg.message + "  | " + moment(msg.time).format("hh:mm") + "<br>")
          });

            //id diffChatrooms to append buttons
            ...
            var localData = database.ref("/location/" + objLocation)
            
            localData.push({
              displayName: displayName,
              message: message,
              time: firebase.database.ServerValue.TIMESTAMP,
              address: object.results[i].vicinity,
              location: placeLoc
            });
          };
        });
